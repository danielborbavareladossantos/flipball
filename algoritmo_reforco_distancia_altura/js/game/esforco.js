...
class Esforco {
    ...
    reforca = (tipo) => {
        ...
        var index = this.tabela.findIndex(e => 
            e.distancia == ultimaEscolha.distancia && 
            e.altura == ultimaEscolha.altura && 
            e.acao == ultimaEscolha.acao);
        if (tipo)
    ...
    buscaPerto = (lista,p_dist) => {
        ...
        nLista.forEach((element,index) => {
            if (Math.abs(element.distancia-p_dist) < min) {
                min = Math.abs(element.distancia-p_dist);
                i = index;
            }
        });
    ...
    check = (distancia,altura) => {
        ...
        if (this.tabela.length>0) {
            ...
            if (closest != null && Math.abs(closest.distancia-distancia) < 5 && Math.abs(closest.altura-altura) < 5) {
                var rLista = this.tabela.filter(e => e.distancia == closest.distancia && e.altura == closest.altura);
        ...
        //faz insercoes
        var tab = this.tabela.filter(e => e.distancia == distancia && e.altura == altura);
        if (!(tab.length>0)) {
